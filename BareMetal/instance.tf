...
module "instances" {
  ...
  display_name           = join("_", [var.bm_prefix], [each.value.display_name])
  ...
  gbs_for_vm         = var.gbs_for_vm
  vlan_tag           = merge(module.vlans.*...)[each.value.vlan_id]["vlans_detail"]["vlan_tag"]
  vlan_cidr_block    = var.vlan_cidr_block
  public_ip_lifetime = each.value.lifetime

  vnic_display_name = join("_", [var.bm_prefix], [each.value.vnic_display_name])
  vnic_vlan_id = each.value.vlan_id != "" ? (length(regexall("ocid1*", each.value.vlan_id)) > 0 ? each.value.vlan_id : merge(module.vlans.*...)[each.value.vlan_id]["vlans_detail"]["vlan_id"]) : null

  secrets_compartment_id = each.value.secrets_compartment_id != null ? (length(regexall("ocid1.compartment.oc1*", each.value.secrets_compartment_id)) > 0 ? each.value.secrets_compartment_id : var.compartment_ocids[each.value.secrets_compartment_id]) : null
