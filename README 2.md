...
2. All the terraform templates (Identity, Network, Security, Compute, BareMetal) are uploaded as private templates in the "<compartmetn> (root)" compartment.
3. Resource Manager Stacks for each of the private templates are created in the "compartment (root)" compartment.

Execution Steps (when run on a server):
1. Create the Identity Components (Compartments, Groups, Dynamic Groups, Policies)
   - Compartment: compartment (root)
   ...
   - Commands to execute via Resource Manager: 
     ...
     - Choose the "compartment (root)" Compartment from the compartment picker. To do this, under List Scope in the Compartment list, click "compartment (root)" compartment.
...
2. Create the Security Components (Vault, Master Encryption Key, Secrets)
   ...
   - Store your private key (of SSH Key pair in PEM/OpenSSH format) in the vault - This key will be used to login to the Bare Metal server as well as the VMs.
     ...
     - From the list of vaults in the compartment, Click "compartment-Vault" to use the existing vault.
     ...
     - Choose the master encryption key - "compartment-MasterEncryptionKey".
...
5. Create the Bare Metal Server along with the desired number of GPU slices/VMs, VLAN, Public IPs and Private IPs.
   ...
   - Steps to provision a Stack from Private Template:
      ...
      b. Choose the "compartment (root)" Compartment from the compartment picker. Select "BareMetal" from the list of templates.
   ...
   - Inputs: (To be given when prompted during terraform plan/apply via terminal or while editing the Resource Manager Stack)
     ...
     - Change the name of the secret (secrets_name = "bm-ssh-key"), if you are using a new one in compartment_bare_metal_instances.auto.tfvars. Defaults to "bm-ssh-key".
